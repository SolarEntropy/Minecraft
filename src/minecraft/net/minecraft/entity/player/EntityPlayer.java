...
public abstract class EntityPlayer extends EntityLivingBase
{
    ...
    public void onLivingUpdate()
    {
        ...
        if (this.getHealth() > 0.0F && !this.isSpectator())
        {
            ...
            {
                axisalignedbb = this.getEntityBoundingBox().merge(this.getRidingEntity().getEntityBoundingBox()).expand(1.0D, 0.0D, 1.0D);
            }
            else
            {
    ...
    {
        float f = this.inventory.getStrVsBlock(p_184813_1_);
    ...
    {
        return this.inventory.canToolBreakBlock(p_184823_1_);
    }
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity)
    {
        if (targetEntity.canAttackWithItem())
        {
            if (!targetEntity.hitByEntity(this))
            {
                ...
                float f2 = this.getAttackTime(0.5F);
                f = f * (0.2F + f2 * f2 * 0.8F);
                f1 = f1 * f2;
                this.resetAttack();
    ...
    public float getAttackTime(float addition)
    {
        return MathHelper.clamp_float(((float)this.attackTime + addition) / this.getAttackSpeed(), 0.0F, 1.0F);
    }
